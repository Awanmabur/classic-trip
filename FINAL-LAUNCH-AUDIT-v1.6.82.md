# Classic Trip v1.6.82 — Checkout / Pesapal Handoff Final Audit
...
Pesapal API 3.0 requires SubmitOrderRequest to return a `redirect_url`; the merchant must redirect the customer to that URL or load it in an iframe. v1.6.82 makes that handoff explicit and retry-safe.
...
- Bus listing-card amenity lanes reserve 62px and stack above the price/actions footer so the second amenity row stays visible.
...
The dependency graph is unchanged from v1.6.80. The user's immediately preceding local v1.6.80 run passed 115/115 tests and reported 0 vulnerabilities. This artifact container's `npm ci` produced incomplete package stubs and `npm test` could not load `mongoose`, so the dependency-backed suite is not claimed for v1.6.82 here. Run `npm ci`, `npm test`, and `npm audit --omit=dev --audit-level=moderate` on the extracted v1.6.82 tree as the final dependency-backed check.
