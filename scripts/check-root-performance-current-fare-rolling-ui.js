...
check('Package and browser cache stay aligned', (Number(pkg.version.split('.')[0]) > 1 || Number(pkg.version.split('.')[1]) > 6 || Number(pkg.version.split('.')[2]) >= 11) && sw.includes(`classic-trip-static-v${pkg.version}`));
