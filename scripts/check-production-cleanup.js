...
check('release is v1.6.65 and lockfile matches',()=>assert(pkg.version==='1.6.65'&&lock.version===pkg.version&&lock.packages?.['']?.version===pkg.version));
...
check('historical launch audits are removed',()=>assert(!fs.readdirSync(root).some(n=>/^FINAL-LAUNCH-AUDIT-v1\.6\.(?!65\b)/.test(n))));
check('only current production audit is present',()=>assert(fs.existsSync(path.join(root,'FINAL-LAUNCH-AUDIT-v1.6.65.md'))));
...
check('stable production checks exist',()=>{assert(scripts['check:public-performance']);assert(scripts['check:runtime-network']);assert(scripts['check:production-cleanup']);});
