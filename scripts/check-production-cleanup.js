...
check('release is v1.6.81 and lockfile matches',()=>assert(pkg.version==='1.6.81'&&lock.version===pkg.version&&lock.packages?.['']?.version===pkg.version));
...
check('only current production audit is present',()=>assert(fs.existsSync(path.join(root,'FINAL-LAUNCH-AUDIT-v1.6.81.md'))));
