...
expect('MongoDB uses an explicit pool', db.includes('minPoolSize: effectivePoolMin') && db.includes('maxPoolSize: effectivePoolMax'));
...
expect('homepage cache warms only after the web port opens', server.includes('catalogService.prewarmHome()') && server.indexOf('app.listen') < server.indexOf('catalogService.prewarmHome()'));
