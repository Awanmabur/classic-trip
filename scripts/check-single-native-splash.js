...
check('native launch identity contains the app name and slogan', () => assert(manifest.name.includes('Classic Trip') && manifest.name.includes('Move, stay and fly with confidence.')));
check('native launch uses the transparent Classic Trip symbols', () => assert(Array.isArray(manifest.icons) && manifest.icons.length >= 2 && manifest.icons.every((icon) => icon.purpose !== 'maskable') && manifest.icons.some((icon) => icon.src.includes('logo-symbol-192.png')) && manifest.icons.some((icon) => icon.src.includes('logo-symbol-512.png'))));
