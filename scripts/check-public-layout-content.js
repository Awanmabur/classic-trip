...
check('release is v1.6.87', () => assert.strictEqual(pkg.version, '1.6.87'));
...
check('Stay bar media stretches to the full row height', () => {
  assert(css.includes('v1.6.87: Stay bars must use the same full-height media column'));
...
check('Bus Card and Bar amenity lanes stay in front without added spacing', () => {
  assert(css.includes('v1.6.87 — final bus amenity layering: no spacer is added below amenities'));
  ...
  assert(css.includes('v1.6.87 exact-fit guard'));
  assert(css.includes('padding-block:0!important'));
  assert(!css.includes('flex:0 0 62px!important'));
  assert(!css.includes('margin:2px 0 8px!important'));
});
check('notification cards keep readable light-mode contrast on public and dashboard pages', () => {
  const notifications = read('public/js/notifications.js');
  assert(notifications.includes('background:var(--panel,var(--card,#0b1220))'));
  assert(notifications.includes('html[data-theme=\"light\"] .ctNotifyPanel{background:#fff;color:#0f172a}'));
  assert(notifications.includes('html[data-theme=\"light\"] .ctNotifyPageItem{background:rgba(15,23,42,.045);color:#0f172a}'));
});
