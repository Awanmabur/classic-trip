...
check('Stay bar media stretches to the full row height', () => {
  assert(css.includes('v1.6.86: Stay bars must use the same full-height media column'));
...
check('Bus Card and Bar amenity lanes stay in front without added spacing', () => {
  assert(css.includes('v1.6.86 — final bus amenity layering: no spacer is added below amenities'));
  ...
  assert(css.includes('v1.6.86 exact-fit guard'));
