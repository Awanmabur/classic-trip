...
check('Render pre-deploy runs launch doctor before index reconciliation', render.includes('preDeployCommand: npm run release:launch && npm run db:indexes'));
check('Render web build runs the full release gate and prunes development dependencies', render.includes('npm run release:check && npm prune --omit=dev'));
