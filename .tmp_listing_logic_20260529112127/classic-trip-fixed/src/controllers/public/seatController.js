...
const { getTenantAccessByCatalog } = require("../../services/tenant/runtime");
const { catalogMatchesTenantScope } = require("../../services/tenant/runtime");
const { syncTripCatalogByTrip } = require("../../services/platform/catalog");
const { SEAT_HOLD_MINUTES } = require("../../config/app");

// public seat map
exports.getSeatMap = asyncHandler(async (req, res) => {
  const { catalog, models } = await getTenantAccessByCatalog(req.params.tripId);
  if (!catalog || !models || !catalogMatchesTenantScope(catalog, req.tenant)) {
    return res.status(404).json({ ok: false, message: "Trip not found" });
  }
  const { Trip, Vehicle, SeatHold, SeatBooking } = models;

  const trip = await Trip.findById(catalog.sourceTripId).lean();
...
exports.holdSeats = asyncHandler(async (req, res) => {
  ...
  const { catalog, tenant, models } = await getTenantAccessByCatalog(req.params.tripId);
  if (!catalog || !models || !catalogMatchesTenantScope(catalog, req.tenant)) {
    return res.status(404).json({ ok: false, message: "Trip not available" });
  }
  const { Trip, SeatHold, SeatBooking } = models;
...
exports.releaseHolds = asyncHandler(async (req, res) => {
  ...
  const { Trip, SeatHold } = models;

  const q = { tripId: catalog.sourceTripId, userId: req.user.userId };
