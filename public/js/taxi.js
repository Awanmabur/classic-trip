(function () {
  ...
  function markerIcon(kind, label) {
    ...
    return window.L.divIcon({
      ...
      html: `<span class="ctMapMarker ${esc(kind)}" role="img" aria-label="${esc(label)}"><i class="fa-solid ${kind === 'pickup' ? 'fa-circle-dot' : kind === 'driver' ? 'fa-location-arrow' : kind === 'stop' ? 'fa-circle' : 'fa-location-dot'}" aria-hidden="true"></i></span>`,
      iconSize: [34, 34],
      iconAnchor: [17, 17],
    });
  }
  ...
  async function submitRide(event) {
    ...
    try {
      ...
      const payment = await api(`/api/v1/taxi/rides/${encodeURIComponent(bookingRef)}/payment`, {
        ...
        body: JSON.stringify({ provider: form.provider.value, idempotencyKey: paymentIdem, lookupCode }),
      });
