...
(() => {
  ...
  function renderTravelerForms(counts) {
    ...
    $('#flightTravelers').innerHTML = types.map((type, index) => `<section class="travelerCard" data-traveler-index="${index}"><div class="travelerTitle"><h3>Traveler ${index + 1}</h3><span class="badge badgeInfo">${esc(type)}</span></div><input type="hidden" name="passengerType" value="${esc(type)}"><div class="travelFormGrid three"><div class="travelField"><label>Title</label><div class="travelControl"><select name="title" aria-label="Traveler ${index + 1} title"><option>Mr</option><option>Ms</option><option>Mrs</option><option>Dr</option><option>Child</option></select></div></div><div class="travelField"><label>First name</label><div class="travelControl"><input name="firstName" aria-label="Traveler ${index + 1} first name" required></div></div><div class="travelField"><label>Last name</label><div class="travelControl"><input name="lastName" aria-label="Traveler ${index + 1} last name" required></div></div><div class="travelField"><label>Sex</label><div class="travelControl"><select name="sex" aria-label="Traveler ${index + 1} sex"><option value="male">Male</option><option value="female">Female</option><option value="unspecified">Prefer not to say</option></select></div></div><div class="travelField"><label>Nationality</label><div class="travelControl"><input name="nationality" aria-label="Traveler ${index + 1} nationality" required></div></div><div class="travelField"><label>Document type</label><div class="travelControl"><select name="documentType" aria-label="Traveler ${index + 1} document type"><option value="national_id">National ID</option><option value="passport">Passport</option><option value="travel_document">Travel document</option></select></div></div><div class="travelField"><label>Document number</label><div class="travelControl"><input name="documentNumber" aria-label="Traveler ${index + 1} document number" autocomplete="off" required></div></div><div class="travelField"><label>Document expiry</label><div class="travelControl"><input type="date" name="documentExpiry" aria-label="Traveler ${index + 1} document expiry"></div></div><div class="travelField"><label>Issuing country</label><div class="travelControl"><input name="documentIssuingCountry" aria-label="Traveler ${index + 1} document issuing country"></div></div><div class="travelField"><label>Frequent flyer</label><div class="travelControl"><input name="frequentFlyerNumber" aria-label="Traveler ${index + 1} frequent flyer number"></div></div></div></section>`).join('');
  }

  function collectTravelers() {
    return $$('[data-traveler-index]', $('#flightTravelers')).map((card) => {
      const value = (name) => $(`[name="${name}"]`, card)?.value || '';
      return { passengerType: value('passengerType'), title: value('title'), firstName: value('firstName'), lastName: value('lastName'), sex: value('sex'), nationality: value('nationality'), documentType: value('documentType'), documentNumber: value('documentNumber'), documentExpiry: value('documentExpiry') || null, documentIssuingCountry: value('documentIssuingCountry'), frequentFlyerNumber: value('frequentFlyerNumber') };
    });
  }
  ...
  async function submitOrder(event) {
    ...
    try {
      ...
      const payment = await api(`/api/v1/flights/orders/${encodeURIComponent(bookingRef)}/payment`, { method: 'POST', headers: { 'Idempotency-Key': paymentIdempotency }, body: JSON.stringify({ provider: form.paymentProvider.value, idempotencyKey: paymentIdempotency, lookupCode }) });
