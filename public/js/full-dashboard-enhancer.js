(function () {
  ...
  function ensureModal() {
    ...
    style.textContent = '\n.ctEnhancerToolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:10px 0 14px}.ctEnhancerToolbar input,.ctEnhancerToolbar select{border:1px solid var(--line,rgba(255,255,255,.12));background:var(--input,rgba(255,255,255,.05));color:var(--text,#fff);border-radius:999px;padding:9px 12px;font-weight:800;font-size:12px}.ctEnhancerToolbar button{border:1px solid var(--line,rgba(255,255,255,.12));background:var(--soft,rgba(255,255,255,.06));color:var(--text,#fff);border-radius:999px;padding:9px 12px;font-weight:900}.ctDisabledAction{opacity:.48;cursor:not-allowed!important;filter:grayscale(.4)}.ctDetailsModal{position:fixed;inset:0;z-index:9999;display:none;background:rgba(0,0,0,.64);backdrop-filter:blur(8px);padding:22px;overflow:auto}.ctDetailsModal.is-open{display:grid;place-items:start center}.ctDetailsPanel{width:min(980px,96vw);max-height:92vh;overflow:auto;border:1px solid var(--line,rgba(255,255,255,.12));border-radius:26px;background:linear-gradient(180deg,var(--panel,#081126),var(--bg1,#050814));box-shadow:0 24px 80px rgba(0,0,0,.32);color:var(--text,#fff)}.ctDetailsHead{position:sticky;top:0;background:inherit;border-bottom:1px solid var(--line,rgba(255,255,255,.12));padding:16px;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;z-index:2}.ctDetailsHead h3{margin:0;font-size:20px}.ctDetailsHead p{margin:4px 0 0;color:var(--muted,rgba(255,255,255,.62));font-weight:700}.ctDetailsBody{padding:16px;display:grid;gap:14px}.ctDetailGroup{border:1px solid var(--line2,rgba(255,255,255,.08));border-radius:20px;background:var(--table,rgba(255,255,255,.03));padding:14px}.ctDetailGroup h4{margin:0 0 10px;font-size:14px}.ctDetailGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px}.ctDetailItem{border:1px solid var(--line2,rgba(255,255,255,.07));border-radius:14px;padding:10px;background:rgba(255,255,255,.025)}.ctDetailItem span{display:block;color:var(--muted,rgba(255,255,255,.62));font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.04em}.ctDetailItem strong{display:block;margin-top:5px;word-break:break-word;font-size:13px}.ctCloseBtn{border:1px solid var(--line,rgba(255,255,255,.12));background:var(--soft,rgba(255,255,255,.06));color:inherit;border-radius:999px;padding:9px 12px;font-weight:900}.ctActionBar{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.ctActionBar button,.ctActionBar a{border:1px solid var(--line,rgba(255,255,255,.12));background:var(--soft,rgba(255,255,255,.06));color:inherit;border-radius:999px;padding:8px 11px;font-weight:900;font-size:12px}.ctActionBar .primary{background:linear-gradient(135deg,#4f8cff,#2f6bff);color:white}.ctHiddenRow{display:none!important}\n';
    document.head.appendChild(style);
    var modal = document.createElement('div');
    modal.id = 'fullDetailsModal';
    modal.className = 'ctDetailsModal';
    modal.innerHTML = '<div class="ctDetailsPanel"><div class="ctDetailsHead"><div><h3 id="ctDetailsTitle">Details</h3><p id="ctDetailsSub">Complete record information</p></div><button type="button" class="ctCloseBtn" data-ct-close>Close</button></div><div class="ctDetailsBody" id="ctDetailsBody"></div></div>';
  ...
  function renderDetails(entry, fallbackTitle) {
    ...
    if (!groups) {
      ...
      if (meta) Object.keys(meta).forEach(function (key) { if (key !== 'details') groups['Table row'][key] = meta[key]; });
    }
    ...
    body += '<div class="ctActionBar"><button type="button" class="primary" data-ct-copy>Copy reference</button><button type="button" data-ct-export-one>Export JSON</button><button type="button" disabled class="ctDisabledAction" title="This action is disabled unless a backend endpoint exists for this exact record state.">State-changing actions only enable when valid</button></div>';
  ...
  function initEnhancer() {
    ...
    document.addEventListener('click', function (event) {
      ...
      modal.querySelector('#ctDetailsSub').textContent = 'Complete grouped record details from the backend dashboard payload.';
