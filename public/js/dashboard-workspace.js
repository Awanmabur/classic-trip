  ...
  function adminFormConfig(type, label = '', detail = {}, mode = 'create') {
    ...
    if (currentRole === 'promoter' && key === 'offline sale') return {
      ...
      fields: [
        ...
        { name:'returnScheduleId', label:'Return departure', type:'select', icon:'fa-rotate', options:schedules, dependsOn:'listingId', filterKey:'listingId', showFor:'bus', help:'Optional. Choose any published future departure that travels back from the outbound destination to its origin; its clock time is independent of the outbound trip.' },
