...
Sensitive stored identifiers and operational tokens are encrypted with AES-256-GCM. v1.6.74 introduced `DATA_ENCRYPTION_KEY` as a dedicated production encryption secret; it must be distinct from `SESSION_SECRET`. Legacy ciphertext remains readable under the preserved session secret for backward compatibility.
