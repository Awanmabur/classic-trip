...
const env = {
  ...
  support: {
    phone: process.env.SUPPORT_PHONE || '+256781977217',
  ...
  push: {
    ...
    subject: process.env.PUSH_VAPID_SUBJECT || `mailto:${process.env.SUPPORT_EMAIL || 'support@classictrip.org'}`,
  },
  seo: {
    siteUrl: process.env.SITE_URL || process.env.APP_URL || 'http://localhost:5000',
    defaultTitle: process.env.SEO_DEFAULT_TITLE || 'Classic Trip | Book Travel Across East Africa',
    defaultDescription: process.env.SEO_DEFAULT_DESCRIPTION || 'Search and book verified buses, stays, flights, local rides, tours, car rentals and cargo across East Africa with live availability and secure payments.',
