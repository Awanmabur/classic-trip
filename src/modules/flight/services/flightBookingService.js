...
function travelerRows(payload = {}, expected) {
  ...
  return rows.map((item, index) => {
    ...
    const dob = new Date(item.dateOfBirth); if (Number.isNaN(dob.getTime()) || dob >= now()) throw validationError(`Traveler ${index + 1} date of birth is invalid`);
...
async function createOrder(payload = {}, actor = {}) {
  ...
  const result = await repo.withTransaction(async (session) => {
    ...
    const passengerSnapshots=persistedTravelers.map((traveler)=>({id:traveler.id,fullName:`${traveler.firstName} ${traveler.lastName}`,name:`${traveler.firstName} ${traveler.lastName}`,nationality:traveler.nationality,dateOfBirth:traveler.dateOfBirth,sex:traveler.sex,identityType:traveler.documentType,identityNumber:`••••${traveler.documentNumberLast4}`,seatNumber:assignments.find(a=>a.travelerId===traveler.id)?.seatNumber||'',seatOrRoom:assignments.find(a=>a.travelerId===traveler.id)?.seatNumber||'',checkInStatus:'not_checked'}));
